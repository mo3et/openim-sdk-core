...
func (g *Group) CreateGroup(ctx context.Context, req *sdkpb.CreateGroupReq) (*sharedpb.IMGroup, error) {
	resp, err := g.createGroup(ctx, &group.CreateGroupReq{
		...
		GroupInfo: &sdkws.GroupInfo{
			...
			NeedVerification:       int32(req.GetNeedVerification()),
			LookMemberInfo:         int32(req.GetLookMemberInfo()),
			ApplyMemberFriend:      int32(req.GetApplyMemberFriend()),
			NotificationUpdateTime: req.GetNotificationUpdateTime(),
			NotificationUserID:     req.GetNotificationUserID(),
		},
	})
	if err != nil {
		return nil, err
